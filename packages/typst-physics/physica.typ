...
#let partialderivative(..sink) = {
  ...
  assert(
    args.len() >= 2,
    message: "expecting one function name and at least one variable name",
  )
...
#let hbar = (
  sym.wj,
  move(dy: -0.08em, strike(offset: -0.55em, extent: -0.05em, sym.planck)),
  sym.wj,
).join()
...
#let __signal_element(e, W, color) = {
  ...
  if e == "&" {
    ...
    return rect(width: 1em, height: W, stroke: (
      left: style,
      top: style,
      right: style,
    ))
  } else if e == "u" {
    return rect(width: W, height: 1em, stroke: (
      left: style,
      bottom: style,
      right: style,
    ))
    ...
    return curve(
      stroke: style,
      curve.move((0em, 1em)),
      curve.quad((W * 20%, 0.2em), (W, 0em)),
    )
  } else if e == "D" {
    return curve(
      stroke: style,
      curve.move((0em, 0em)),
      curve.quad((W * 20%, 0.8em), (W, 1em)),
    )
  } else if e == "X" {
    return curve(
      stroke: style,
